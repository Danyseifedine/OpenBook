...
class BaseController extends Controller
{
    ...
    function Select_column(
        $type = ['paginate', 'first', 'Random_order_with_paginate'],
        $model,
        $ColumnName,
        $where = [],
        $paginateNumber = 12,
        ...
        if ($type == 'first') {
            ...
            return $datas;
        } elseif ($type == 'Random_order_with_paginate') {
            $datas = $model::select($ColumnName)->where($where)->inRandomOrder()->paginate($paginateNumber);
            return $datas;
        }
    }
